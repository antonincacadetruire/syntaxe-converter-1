...
let parse_antlr_to_tree (antlr_source : string) : grammar =
  ...
  try
    ...
    Antlr_parser.main Antlr_lexer.token lexbuf
  with
  | Parsing.Parse_error ->
      let loc = {
      ...
      let end_pos = min (String.length antlr_source) (lexbuf.Lexing.lex_curr_pos + 20) in
      let context = String.sub antlr_source start_pos (end_pos - start_pos) in
      raise (Parse_error {
